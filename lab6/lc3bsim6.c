...
void MEM_stage() {
    ...
    int ready = 0, output = 0;
    if (PS.MEM_V & Get_DCACHE_EN(PS.MEM_CS)) {
        int WE0 = 0, WE1 = 0;
        if (Get_DCACHE_RW(PS.MEM_CS)) {
            if (Get_DATA_SIZE(PS.MEM_CS)) {
                ...
                WE0 = (PS.MEM_ADDRESS & 1) ? 0 : 1;
                WE1 = !WE0;
            }
        }
        int input = Get_DATA_SIZE(PS.MEM_CS) ? PS.MEM_ALU_RESULT :
                    (bitsToDec(PS.MEM_ALU_RESULT, 7, 0, UNSIGNED) << 8) + bitsToDec(PS.MEM_ALU_RESULT, 7, 0, UNSIGNED);
        dcache_access(PS.MEM_ADDRESS, &output, input, &ready, WE0, WE1);
    }
    if (!Get_DATA_SIZE(PS.MEM_CS)) {
        output = Low16bits((PS.MEM_ADDRESS & 1) ? bitsToDec(output, 15, 8, SIGNED) : bitsToDec(output, 7, 0, SIGNED));
    }
    trap_pc = output;
    target_pc = PS.MEM_ADDRESS;
    mem_stall = (PS.MEM_V & Get_DCACHE_EN(PS.MEM_CS)) & !ready;
    if (PS.MEM_V & Get_BR_OP(PS.MEM_CS)) {
        int BEN = (bitsToDec(PS.MEM_CC, 2, 2, UNSIGNED) & (bitsToDec(PS.MEM_IR, 11, 11, UNSIGNED))) ||
                  (bitsToDec(PS.MEM_CC, 1, 1, UNSIGNED) & (bitsToDec(PS.MEM_IR, 10, 10, UNSIGNED))) ||
                  (bitsToDec(PS.MEM_CC, 0, 0, UNSIGNED) & (bitsToDec(PS.MEM_IR, 9, 9, UNSIGNED)));
        if (BEN) {
            mem_pcmux = 1;
        }
    }
...
void AGEX_stage() {
    ...
    int addr1_mux = Get_ADDR1MUX(PS.AGEX_CS) ? PS.AGEX_SR1 : PS.AGEX_NPC;
    int adder;
    switch (Get_ADDR2MUX(PS.AGEX_CS)) {
        case 0:
            adder = Low16bits(addr1_mux + (Get_LSHF1(PS.AGEX_CS) ? 0 << 1 : 0));
            break;
        case 1:
            adder = Low16bits(addr1_mux + (Get_LSHF1(PS.AGEX_CS) ?
                                           bitsToDec(PS.AGEX_IR, 5, 0, SIGNED) << 1 : bitsToDec(PS.AGEX_IR, 4, 0, SIGNED)));
            break;
        case 2:
            adder = Low16bits(addr1_mux + (Get_LSHF1(PS.AGEX_CS) ?
                                           bitsToDec(PS.AGEX_IR, 8, 0, SIGNED) << 1 : bitsToDec(PS.AGEX_IR, 4, 0, SIGNED)));
            break;
        case 3:
            adder = Low16bits(addr1_mux + (Get_LSHF1(PS.AGEX_CS) ?
                                           bitsToDec(PS.AGEX_IR, 10, 0, SIGNED) << 1 : bitsToDec(PS.AGEX_IR, 4, 0, SIGNED)));
    }
    int address_mux = Get_ADDRESSMUX(PS.AGEX_CS) ? adder : bitsToDec(PS.AGEX_IR, 7, 0, UNSIGNED) << 1;
    int shifter;
    if (!bitsToDec(PS.AGEX_IR, 4, 4, UNSIGNED)) {
        shifter = Low16bits(PS.AGEX_SR1 << bitsToDec(PS.AGEX_IR, 3, 0, UNSIGNED));
    }
    else if (bitsToDec(PS.AGEX_IR, 5, 5, UNSIGNED)) { // RSHFA
        if (bitsToDec(PS.AGEX_SR1, 15, 15, UNSIGNED)) {
            int TEMP = PS.AGEX_SR1;
            for (int i = bitsToDec(PS.AGEX_IR, 3, 0, UNSIGNED); i > 0; i--) {
                TEMP = (TEMP >> 1) + 0x8000;
            }
            shifter = Low16bits(TEMP);
        }
        else {
            shifter = Low16bits(PS.AGEX_SR1 >> bitsToDec(PS.AGEX_IR, 3, 0, UNSIGNED));
        }
    }
    else if (!bitsToDec(PS.AGEX_IR, 5, 5, UNSIGNED)) { // RSHFL
        shifter = Low16bits(PS.AGEX_SR1 >> bitsToDec(PS.AGEX_IR, 3, 0, UNSIGNED));
    }
    int sr2_mux = Get_SR2MUX(PS.AGEX_CS) ? bitsToDec(PS.AGEX_IR, 4, 0, SIGNED) : PS.AGEX_SR2;
    int alu;
    switch (Get_ALUK(PS.AGEX_CS)) {
        case 0:
            alu = Low16bits(PS.AGEX_SR1 + sr2_mux);
            break;
        case 1:
            alu = Low16bits(PS.AGEX_SR1 & sr2_mux);
            break;
        case 2:
            ...
            break;
        case 3:
            alu = Low16bits(sr2_mux);
    }
    int alu_result_mux = Get_ALU_RESULTMUX(PS.AGEX_CS) ? alu : shifter;
    ...
    if (LD_MEM) {
        ...
        NEW_PS.MEM_ADDRESS = address_mux;
        NEW_PS.MEM_NPC = PS.AGEX_NPC;
        NEW_PS.MEM_CC = PS.AGEX_CC;
        NEW_PS.MEM_ALU_RESULT = alu_result_mux;
...
void DE_stage() {
    ...
    int CONTROL_STORE_ADDRESS = (bitsToDec(PS.DE_IR, 15, 11, UNSIGNED) << 1) + bitsToDec(PS.DE_IR, 5, 5, UNSIGNED);
    ...
    int DR = Get_DRMUX(CONTROL_STORE[CONTROL_STORE_ADDRESS]) ? 7 : bitsToDec(PS.DE_IR, 11, 9, UNSIGNED);
    int SR1 = bitsToDec(PS.DE_IR, 8, 6, UNSIGNED);
    int SR2 = bitsToDec(PS.DE_IR, 13, 13, UNSIGNED) ?
              bitsToDec(PS.DE_IR, 11, 9, UNSIGNED) : bitsToDec(PS.DE_IR, 2, 0, UNSIGNED);

    if (PS.DE_V &&
        (((Get_SR1_NEEDED(CONTROL_STORE[CONTROL_STORE_ADDRESS])) && (v_agex_ld_reg & (PS.AGEX_DRID == SR1)) | (v_mem_ld_reg & (PS.MEM_DRID == SR1)) | (v_sr_ld_reg & (PS.SR_DRID == SR1))) ||
         ((Get_SR2_NEEDED(CONTROL_STORE[CONTROL_STORE_ADDRESS])) && (v_agex_ld_reg & (PS.AGEX_DRID == SR2)) | (v_mem_ld_reg & (PS.MEM_DRID == SR2)) | (v_sr_ld_reg & (PS.SR_DRID == SR2))) ||
         ((Get_DE_BR_OP(CONTROL_STORE[CONTROL_STORE_ADDRESS])) && (v_agex_ld_cc | v_mem_ld_cc | v_sr_ld_cc)))) {
        dep_stall = 1;
    }
    else {
        dep_stall = 0;
...
void FETCH_stage() {
    ...
    if (icache_r & (!mem_stall & !v_de_br_stall & !v_agex_br_stall & !v_mem_br_stall & !dep_stall) | (v_mem_br_stall & (mem_pcmux != 0))) {
        switch (mem_pcmux) {
            case 0:
                PC = PC + 2;
                break;
